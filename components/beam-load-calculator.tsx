...
export function BeamLoadCalculatorComponent() {
  ...
  const calculateResults = useCallback(() => {
    ...
    if (beamType === 'Simple Beam') {
      if (loadType === 'Point Load') {
        R1 = loadMagnitude * (rightSupportM - loadStartPositionM) / (rightSupportM - leftSupportM)
        R2 = loadMagnitude * (loadStartPositionM - leftSupportM) / (rightSupportM - leftSupportM)
      } else if (loadType === 'Uniform Load') {
        const loadLengthM = loadEndPositionM - loadStartPositionM
        const totalLoad = loadMagnitude * loadLengthM
    ...
    if (beamType === 'Simple Beam') {
      if (loadType === 'Point Load') {
        maxBendingMoment = R1 * (loadStartPositionM - leftSupportM)
      } else if (loadType === 'Uniform Load') {
        const loadLengthM = loadEndPositionM - loadStartPositionM
        maxBendingMoment = (R1 * (loadStartPositionM - leftSupportM)) - 
                           (loadMagnitude * loadLengthM * (loadStartPositionM - leftSupportM + loadLengthM / 2) / 2)
      }
    } else if (beamType === 'Cantilever Beam') {
      if (loadType === 'Point Load') {
        maxBendingMoment = loadMagnitude * (beamLengthM - loadStartPositionM)
      } else if (loadType === 'Uniform Load') {
        const loadLengthM = loadEndPositionM - loadStartPositionM
        maxBendingMoment = loadMagnitude * loadLengthM * (beamLengthM - (loadStartPositionM + loadEndPositionM) / 2)
      }
    }
  ...
  }, [beamType, beamLength, leftSupport, rightSupport, loadType, loadMagnitude, loadStartPosition, loadEndPosition]);
...
You're right, and I apologize for the confusion. I'll remove the summary from the code and complete it properly. Here's the corrected and completed version of the `BeamLoadCalculator` component:

```tsx project="Beam Load Calculator" file="beam-load-calculator.tsx" type="react"
'use client'
    ...
    try {
      ...
      pdf.autoTable({
        ...
        body: [
          ...
          ['Max Bending Moment', results.maxBendingMoment.toFixed(2), 'N·m'],
